...
let update (msg : Msg) (currentModel : Model) : Model * Cmd<Msg> =
  match currentModel.description, msg with
    | Some desc, UpdateHeight h ->
      let nextModel = { currentModel with description = Some { height = int32 h; weight = desc.weight } }
      (nextModel, Cmd.none)
    | Some desc, UpdateWeight w ->
      let nextModel = { currentModel with description = Some { height = desc.height; weight = int32 w } }
      (nextModel, Cmd.none)
    | _ ->
  ...
  div [] [
    ...
    Container.container [] [
      ...
      BmiForm.form {
        ...
        onHeightChange = (fun e -> dispatch (UpdateHeight !!e.target?value))
        onWeightChange = (fun e -> dispatch (UpdateWeight !!e.target?value))
      }
    ]
